...
/*calculating defect count and grouping by product family*/
WITH Defects AS (  /*creating the CTE*/
    SELECT 
        ...
        END AS Is_Defect
    FROM Measurements m
    JOIN Specifications s ON m.Product_Code = s.Product_Code /*joining specification table to Measurements on product code*/
    JOIN (
...
/*calculating defect count and grouping by product family and lot number*/
WITH Defects AS (   /*creating the CTE*/
    SELECT 
        ps.Product_Family,
        m.Lot_No,
...
FROM Defects
GROUP BY Product_Family, Lot_No  /*grouping the result by product family and lot number*/
ORDER BY Defect_Count DESC;      
